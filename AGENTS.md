...
Use this guide for automated work in the `surgeist-shape` crate repo.
...
This repo owns the `surgeist-shape` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It links this repo as a submodule under `crates/surgeist-shape` and owns workspace wiring, submodule pointers, cross-crate plans, source-derived API coordination, and whole-system verification.
...
`surgeist-shape` owns shape, geometry, and primitive path data.
...
1. Confirm the work belongs in `surgeist-shape`.
